...
/**
 * @file
 * Read-only report: nodes missing field_main_image.
 *
 * Usage:
 *   ddev drush php:script scripts/missing-image-report.php
 *   ddev drush php:script scripts/missing-image-report.php -- --format=csv
 */

use Drupal\node\Entity\Node;

$bundles = ['stallion', 'container_home'];
$format = 'markdown';
...
print "# Missing main image report\n\n";
print 'Generated: ' . date('c') . "\n\n";
print 'Read-only. Does **not** attach or guess media.' . "\n\n";
print '| nid | bundle | title | status | updated | alias |' . "\n";
print '|-----|--------|-------|--------|---------|-------|' . "\n";
foreach ($rows as $row) {
  print sprintf(
